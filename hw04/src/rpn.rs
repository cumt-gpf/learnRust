...
impl Stack {
    ...
    pub fn eval(&mut self, op: Op) -> Result<()> {
        ...
        match op {
            ...
            Op::Swap => {
                let x: Elt = try!(self.pop());
                let y: Elt = try!(self.pop());
                self.push(x);
                self.push(y);
